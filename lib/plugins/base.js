...
var mapArtistTitle = require('../').mapArtistTitle
...
function cleanMVPV (string) {
  return string
    .replace(/\s*\[\s*([PM]\/?V)\s*]/, '') // [MV] or [M/V]
    .replace(/\s*\(\s*([PM]\/?V)\s*\)/, '') // (MV) or (M/V)
    .replace(/\s*【\s*([PM]\/?V)\s*】/, '') // 【MV】 or 【M/V】
    .replace(/[\s\-–_]+([PM]\/?V)\s*/, '') // MV or M/V at the end
    .replace(/([PM]\/?V)[\s\-–_]+/, '') // MV or M/V at the start
}
...
var plugin = combinePlugins([
  ...
  { before: cleanFluff,
    split: splitArtistTitle,
    after: mapArtistTitle(cleanArtist, cleanTitle) },
  require('./quoted-title'),
  require('./common-fluff')
])
exports.before = plugin.before
exports.split = plugin.split
exports.after = plugin.after
